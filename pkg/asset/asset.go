...
// EnsureAssetDirExists ensures that the asset directory exists
...
func AddAsset(projectPath, assetPath string, assetType AssetType) error {
	...
	if err := EnsureAssetDirExists(projectPath); err != nil {
		return fmt.Errorf("failed to ensure asset directory exists: %v", err)
	}
...
func updatePubspecWithAsset(projectPath string, assetType AssetType) error {
	...
	if !ok {
		flutter = make(map[string]interface{})
	...
	for _, asset := range assets {
		if asset.(string) == assetPath {
			exists = true
			break
		}
	}

	// Add the asset path if it doesn't exist
	if !exists {
	...
	updatedPubspecData, err := yaml.Marshal(pubspec)
	if err != nil {
		return fmt.Errorf("failed to marshal pubspec.yaml: %v", err)
	}

	if err := os.WriteFile(pubspecPath, updatedPubspecData, 0644); err != nil {
